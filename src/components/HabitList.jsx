...
const DatePickerHeader = ({
  ...
  return (
    <div className="relative flex items-center justify-between mb-4 px-4 pt-4 sm:px-6 sm:pt-6 border-b dark:border-gray-700 pb-4">
      ...
      <Button
        onClick={onAddHabit}
        className="bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white shadow-md h-9 px-3 ml-2"
        size="sm"
      >
  ...
  return (
    <Card className={`bg-white/90 dark:bg-gray-950/90 flex flex-col flex-grow relative`}>
      ...
      {isCalendarOpen && (
        <div
          ref={calendarRef}
          className="absolute top-16 left-4 sm:left-6 z-20 mt-1 w-80 max-w-[calc(100vw-2rem)] rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none"
          role="dialog"
          aria-modal="true"
        >
          <div className="p-3">
            <CalendarCard
              selectedDate={selectedDate}
              onDateSelect={handleInlineDateSelect}
